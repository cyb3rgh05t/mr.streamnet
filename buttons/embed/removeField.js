    ...
    async execute(interaction, client) {
        ...
        db.findOne({
            ...
            if (Row5.components.length >= 1) {
                interaction.reply({
                    components: [Row1, Row2, Row3, Row4, Row5, Row0]
                }).catch((err) => console.error(err.message));
            } else if (Row4.components.length >= 1) {
                interaction.reply({
                    components: [Row1, Row2, Row3, Row4, Row0]
                }).catch((err) => console.error(err.message));
            } else if (Row3.components.length >= 1) {
                interaction.reply({
                    components: [Row1, Row2, Row3, Row0]
                }).catch((err) => console.error(err.message));
            } else if (Row2.components.length >= 1) {
                interaction.reply({
                    components: [Row1, Row2, Row0]
                }).catch((err) => console.error(err.message));
            } else {
                interaction.reply({
                    components: [Row1, Row0]
                }).catch((err) => console.error(err.message));
            }
            ...
            if (PrevEmbed.author == null) {
                PrevEmbed2.setAuthor({
                    name: "  "
                })
            } else {
                if (PrevEmbed.author.iconURL == null) {
                    PrevEmbed2.setAuthor({
                        name: `${PrevEmbed.author.name}`
                    })
                } else {
                    PrevEmbed2.setAuthor({
                        name: `${PrevEmbed.author.name}`,
                        iconURL: `${PrevEmbed.author.iconURL}`
                    })
                }
            }

            if (PrevEmbed.footer == null) {
                PrevEmbed2.setFooter({
                    text: "  "
                })
            } else {
                if (PrevEmbed.footer.iconURL == null) {
                    PrevEmbed2.setFooter({
                        text: `${PrevEmbed.footer.text}`
                    })
                } else {
                    PrevEmbed2.setFooter({
                        text: `${PrevEmbed.footer.text}`,
                        iconURL: `${PrevEmbed.footer.iconURL}`
                    })
                }
            }
            ...
            collector.on("collect", async (b) => {
                if (b.member.id != m.id) return interaction.reply({
                    content: "Diese Knöpfe sind nicht für dich!",
                    ephemeral: true
                }).catch((err) => console.error(err.message));
