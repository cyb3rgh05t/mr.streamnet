...
module.exports = {
    ...
    async execute(interaction, client) {
        ...
        if (!Data) return interaction.reply({
            content: "You havn't setup a mod message log yet, Please do this with [/modlogsetup]",
            ephemeral: true
        });
        ...
        if (!id)
            return interaction.reply({
                content: "Possible member already left on there own or something els went wrong",
                ephemeral: true
            })
                ...
                switch (hard) {
                    case "warning": {
                        if (!interaction.member.permissions.has("MANAGE_MESSAGES"))
                            return interaction.reply({
                                content: "You do not have the right permissions to use this action",
                                ephemeral: true
                            });

                        if (interaction.member.roles.highest.position <= id.roles.highest.position)
                            return interaction.reply({
                                content: "You can't use this action on this member, this member is equal to your rank or higher",
                                ephemeral: true
                            })
                        ...
                        const warn = new MessageEmbed()
                            ...
                            .setColor("GREEN")
                            .setThumbnail(id.user.avatarURL({
                                dynamic: true
                            }))
                            .setFields({
                                name: "User ID:",
                                ...
                                value: `<t:${parseInt(id.user.createdTimestamp / 1000)}:R>`,
                                inline: true
                            });
                        await id.send({
                        ...
                        await guild.channels.cache.get(`${channellog}`).send({
                            embeds: [warn]
                        });
                    }
                    break;
